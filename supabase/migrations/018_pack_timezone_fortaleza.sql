-- Migration 018: corrige o fuso do "dia" do pack (Fortaleza, UTC-3)
--
-- Antes: CURRENT_DATE (UTC). A virada do dia caía 00:00 UTC = 21:00 em
-- Fortaleza, então o pack resetava 3h cedo (na noite anterior). Agora o dia
-- usa a data local de Fortaleza, resetando à meia-noite no horário certo.
-- A Edge Function claim-daily-pack também foi ajustada (mesma data local).

-- ── claim_daily_pack: usa data de Fortaleza ────────────────────
...
-- ── get_pack_status: usa data de Fortaleza ─────────────────────
...
BEGIN
  ...
  FROM public.daily_claims
  WHERE user_id   = p_user_id
    AND claim_date = (now() AT TIME ZONE 'America/Fortaleza')::date
    AND status     = 'completed';
